...
rule flye_assembly:
  input: GS.remote(join(TMP, 'reads', '{strain}_long_reads_corrected.fa')),
  output: assembly = GS.remote(join(OUT, 'assemblies', '01_Ac_{strain}_flye.fa'))
  params:
...
#  input: GS.remote(join(OUT, 'assemblies', '01_Ac_{strain}_flye.fa'))
#  output: GS.remote(join(TMP, '{strain}', 'contig_similarity.tsv'))
...
#  input:
#    genome = GS.remote(join(OUT, 'assemblies', '01_Ac_{strain}_flye.fa')),
#    similarity = GS.remote(join(TMP, '{strain}', 'contig_similarity.tsv'))
#  output:
#    keep = GS.remote(join(OUT, 'assemblies', '02_Ac_{strain}_haplofilter.fa')), 
#    drop = GS.remote(join(TMP, '{strain}_drop_haplotypes.fa'))
#  params:
#    similarity = 90
#  run:
